...
int OnInit() {
    ...
    SetIndexBuffer(3, corr2, INDICATOR_CALCULATIONS);
    return (0);
}
...
int OnCalculate(
    ...
    for (int i = (int) MathMax(prev_calculated - 1, 0); i < rates_total; i++) {
        ...
        processPearsonCorr(hpPeriod, avgLength, filt, i, corr, corr2);

        // compute the result
        computeResult(hpPeriod, corr, corr2, result, i);
...
void computeResult(double highPassPeriod, double& inputArr[], double& inputArrLagged[], double& finalResult[], int i){

        if(i > highPassPeriod){
            int sumDeltas = 0;
            for(int lag = 3; lag <= highPassPeriod; lag++){
                if(((inputArr[lag] > 0.5) && (inputArrLagged[lag] < 0.5)) || ((inputArr[lag] < 0.5) && (inputArrLagged[lag] > 0.5))){
...
void processPearsonCorr(int barsLag, int averageLength, double& inputArr[], int i, double& outputArr[], double& outputArrLagged[]) {
    ...
    if (i > (barsLag + 1 + avLength)) {
        ...
        for (int lag = 0; lag <= barsLag; lag++) {
            outputArrLagged[lag] = outputArr[lag];
            ...
            if (((m * Sxx - Sx * Sx) * (m * Syy - Sy * Sy)) > 0) {
                ...
                outputArr[lag] = 0.5 * (outputArr[lag] + 1);
            }
