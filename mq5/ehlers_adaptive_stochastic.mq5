...
int OnInit() {
    ...
    SetIndexBuffer(3, stoch, INDICATOR_CALCULATIONS);
    return (0);
}
...
int OnCalculate(
    ...
    for (int i = (int) MathMax(prev_calculated - 1, 0); i < rates_total; i++) {
        ...
        // stochastic (Result)
        computeStochastic(lpPeriod, hpPeriod, filt, (int)dominantCycle, stoch, result, i);
    }
...
void computeStochastic(int lowPassPeriod, int highPassPeriod, double& inputArr[], int domCycle, double& stochastic[], double& finalResult[], int i){
	...
	if (i > (highPassPeriod + 1)){
    	...
    	finalResult[i] = c1 * (stochastic[i] + stochastic[i - 1]) / 2.0 + c2 * finalResult[i - 1] + c3 * finalResult[i - 2];
