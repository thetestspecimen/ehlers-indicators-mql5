...
double ratio[]; 	// ratio buffer
double avePrices[]; // average prices buffer
...
int OnInit() {
    ...
    SetIndexBuffer(3, ratio, INDICATOR_CALCULATIONS);
    SetIndexBuffer(4, avePrices, INDICATOR_CALCULATIONS);
    return (0);
}
...
int OnCalculate(
    ...
    for (int i = (int) MathMax(prev_calculated - 1, 0); i < rates_total; i++) {
        ...
        // compute the CCI (Result)
        computeCCI(lpPeriod, hpPeriod, filt, (int)dominantCycle, ratio, result, i, avePrices);
    }
...
void computeCCI(int lowPassPeriod, int highPassPeriod, double& inputArr[], int domCycle, double& ratioArr[], double& finalResult[], int i, double& avePriceArr[]){
    ...
    if (i > (highPassPeriod + 1)){
        ...
        for(int count = 0; count < domCycle; count++){
            avePrice = avePrice + inputArr[i - count]; 
        }

        avePrice = avePrice / (double)domCycle;
        avePriceArr[i] = avePrice;

        double rms = 0;

        for(int count1 = 0; count1 < domCycle; count1++){
            rms = rms + pow((inputArr[i - count1] - avePriceArr[i - count1]), 2); 
        }

        rms = MathSqrt(rms / (double)domCycle);
        double num = inputArr[i] - avePrice;
        double denom = 0.015 * rms;
        ratioArr[i] = num / denom;

        finalResult[i] = c1 * (ratioArr[i] + ratioArr[i-1]) / 2 + c2 * finalResult[i - 1] + c3 * finalResult[i - 2];
