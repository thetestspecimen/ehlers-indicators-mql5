...
int OnCalculate(
    ...
    for (int i = (int) MathMax(prev_calculated - 1, 0); i < rates_total; i++) {
        ...
        if(i<3){
            ...
            pwr = (pow(filt[i],2) + pow(filt[i-1],2) + pow(filt[i-2],2)) / 3;
            wave = wave / MathSqrt(pwr);
            result[i] = wave;
        }
    }
...
// high pass filter
double processHighPass(int highPassPeriod, double& inputArr[], double& outputArr[], int i) {
