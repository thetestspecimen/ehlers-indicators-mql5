...
double stoch[]; 	// stochastic buffer
double adapStoch[]; // adaptive stochastic result buffer
...
int OnInit() {
    ...
    SetIndexBuffer(4, stoch, INDICATOR_CALCULATIONS);
    SetIndexBuffer(5, adapStoch, INDICATOR_CALCULATIONS);
...
int OnCalculate(
    ...
    for (int i = (int) MathMax(prev_calculated - 1, 0); i < rates_total; i++) {
        ...
        // stochastic (Result)
        computeStochastic(lpPeriod, hpPeriod, filt, (int)dominantCycle, stoch, result, i, adapStoch, signalLine);
    }
...
void computeStochastic(int lowPassPeriod, int highPassPeriod, double& inputArr[], int domCycle, double& stochastic[], double& finalResult[], int i, double& adapStochastic[], double& signal[]){
	...
	if (i > (highPassPeriod + 1)){
		...
		double highestC = inputArr[i];
	    double lowestC = inputArr[i];

	    for (int cycle = 0; cycle < domCycle; cycle++) {
	        if (inputArr[i - cycle] > highestC) {
	            highestC = inputArr[i - cycle];
	        }
	        if (inputArr[i - cycle] < lowestC) {
	            lowestC = inputArr[i - cycle];
	        }
    	}

    	stochastic[i] = (inputArr[i] - lowestC) / (highestC - lowestC);
    	adapStochastic[i] = c1 * (stochastic[i] + stochastic[i - 1]) / 2.0 + c2 * adapStochastic[i - 1] + c3 * adapStochastic[i - 2];
        double value1 = 2 * (adapStochastic[i] - 0.5);
        finalResult[i] = (MathExp(2*3*value1) - 1) / (MathExp(2*3*value1) + 1); 
        signal[i] = 0.9 * finalResult[i-1];
