...
int OnCalculate(
    ...
    for (int i = (int) MathMax(prev_calculated - 1, 0); i < rates_total; i++) {
        ...
        // compute the RSI (Result)
        computeRSI(lpPeriod, hpPeriod, filt, (int)dominantCycle, denom, result, i, closesUpOld);
    }
...
void computeRSI(int lowPassPeriod, int highPassPeriod, double& inputArr[], int domCycle, double& denominator[], double& finalResult[], int i, double& closeUp1){
    ...
    if (i > (highPassPeriod + 1)){
        ...
        if(denominator[i] != 0 && denominator[i-1] != 0){
            finalResult[i] = c1 * (closeUp / denominator[i] + closeUp1 / denominator[i-1]) / 2 + c2 * finalResult[i - 1] + c3 * finalResult[i - 2];
        } else {
            finalResult[i] = 0;
        }
