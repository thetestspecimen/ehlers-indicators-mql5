...
int OnInit() {
    ...
    a1 = MathExp(-1.0 * MathSqrt(2.0) * pi / (double)lpPeriod);
    b1 = 2.0 * a1 * MathCos(MathSqrt(2.0) * pi / (double)lpPeriod);
    c2 = b1;
    c3 = -a1 * a1;
    c1 = 1.0 - c2 - c3;
    return (0);
...
int OnCalculate(
    ...
    for (int i = (int) MathMax(prev_calculated - 1, 0); i < rates_total; i++) {
        ...
        if(i<3){
            ...
            result[i] = c1 * (work[i] + work[i - 1]) / 2.0 + c2 * result[i - 1] + c3 * result[i - 2];
        }
    }
